...
class MainTest(unittest.TestCase):
    def run_main(self, reads, iterations):
        # main() now polls before it sleeps, so the sleep that ends the budget lands on the same
        # iteration as the last poll (not one iteration later) - hence >=, not >, or one extra,
        # unbudgeted poll would run past the end of `reads`.
        performed = {"sleeps": 0}
