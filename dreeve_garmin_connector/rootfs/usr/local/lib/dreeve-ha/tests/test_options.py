...
class ReadTest(unittest.TestCase):
    ...
    def test_missing_file_yields_defaults(self):
        ...
        self.assertEqual(result["garmin_email"], "")
        self.assertEqual(result["extra_env"], [])

    def test_values_are_read_and_stripped(self):
        result = options.read(self.write({"garmin_email": "  me@example.com  "}))

        self.assertEqual(result["garmin_email"], "me@example.com")
    ...
    def test_null_value_falls_back_to_default_shape(self):
        result = options.read(self.write({"mfa_code": None, "extra_env": None}))

        self.assertEqual(result["mfa_code"], "")
        self.assertEqual(result["extra_env"], [])
