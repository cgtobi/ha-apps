...
if [ "${1:-}" = "--in-image" ]; then
  IMAGE="dreeve-wahoo-addon:test"
  docker build -t "$IMAGE" "$ADDON_DIR"
  # PYTHONPATH is passed here rather than baked into the image, so it never joins the upstream
  # connector's own import path - upstream's package is `app`, and an image-wide PYTHONPATH would put
  # this directory in front of it.
  exec docker run --rm --entrypoint /usr/local/bin/python \
